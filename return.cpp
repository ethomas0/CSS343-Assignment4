/*
 * Return Class Implementation File  @file return.cpp
 *
 ...
 *					 stock will be increase by 1, and the transaction
...
void Return::processCommands(char type, ifstream& commandFile) {
	int customerID;
	...
	if (Movie* moviePtr = theShelves.retreiveMovie(genre, commandFile)) {
		...
		//moviePtr-> **SOME FUNCTION TO REDUCE INCREASE STOCK BY 1 **

		transactee.getCustomer(customerID)->newTransaction(type, moviePtr);
	}
	else {
		errorCollection.push_back("Sorry, " + moviePtr->movieInfo() + "is not from our store.");
	}
